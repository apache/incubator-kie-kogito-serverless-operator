...
import (
	...
	klog "github.com/ricardozanini/kogito-builder/util/log"
...
func (r *KogitoServerlessPlatformReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if ok, err := platform.IsOperatorAllowedOnNamespace(ctx, r.Reader, req.Namespace); err != nil {
		...
		logger.Info("Ignoring request because namespace is locked")
	...
	for _, a := range actions {
		...
		if a.CanHandle(target) {
			...
			if err != nil {
				//FIXME Implement a NotifyìPlatformError function
				//NotifyIntegrationPlatformError(ctx, r.Client, r.Recorder, &instance, target, err)
				return reconcile.Result{}, err
			}
			...
			if target != nil {
				...
				if err := r.Client.Status().Patch(ctx, target, ctrl.MergeFrom(&instance)); err != nil {
					//FIXME Implement a NotifyìPlatformError function
					//NotifyPlatformError(ctx, r.Client, r.Recorder, &instance, target, err)
					return reconcile.Result{}, err
				}
			...
			// FIXME Implement a NotifyIPlatformUpdated function
			// NotifyPlatformUpdated(ctx, r.Client, r.Recorder, &instance, target)
			break
		}
	}
