...
func GetLocalPlatform(ctx context.Context, c ctrl.Reader, namespace string, active bool) (*operatorapi.SonataFlowPlatform, error) {
	klog.V(log.D).Info("Finding available platforms")
	...
	for _, platform := range lst.Items {
		...
		if IsActive(&platform) {
			klog.V(log.D).Infof("Found active local build platform %s", platform.Name)
			return &platform, nil
		}
	}
	...
	if !active && len(lst.Items) > 0 {
		...
		res := lst.Items[0]
		klog.V(log.D).Infof("Found local build platform %s", res.Name)
		return &res, nil
	}

	klog.V(log.D).Infof("Not found a local build platform")
