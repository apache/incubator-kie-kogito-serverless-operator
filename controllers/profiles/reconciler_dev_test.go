...
func Test_newDevProfileWithExternalConfigMaps(t *testing.T) {
	...
	cmLabels["sw.kogito.kie.org/resource-camel"] = "mycamel-configmap"
	cmUser := createConfigMapBase("Test_newDevProfileWithExternalConfigMaps", "mycamel-configmap", cmData, cmLabels)
	...
	assert.Equal(t, extCamel.Name, camelXmlRouteFileName)
	assert.Equal(t, extCamel.MountPath, camelDevConfigMountPath)
	...
	//Now we expect 3 volumes wd, camelroute.xml and camelroute.yaml
	deployment = test.MustGetDeployment(t, client, workflow)
	assert.Equal(t, 4, len(deployment.Spec.Template.Spec.Containers[0].VolumeMounts))
	...
	assert.Equal(t, extCamelRouteOne.Name, camelXmlRouteFileName)
	assert.Equal(t, extCamelRouteOne.MountPath, camelDevConfigMountPath)
	extCamelRouteTwo := deployment.Spec.Template.Spec.Containers[0].VolumeMounts[3]
	assert.Equal(t, extCamelRouteTwo.Name, camelYamlRouteFileName)
	assert.Equal(t, extCamelRouteTwo.MountPath, camelDevConfigMountPath)
...
func createConfigMapBase(namespace string, name string, cmData map[string]string, labels map[string]string) clientruntime.Object {
	cm := &corev1.ConfigMap{
